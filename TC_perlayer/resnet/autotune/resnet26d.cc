...
TEST(resnet, connv)
{
    ...
    time_use_sum = 0;
    at::Tensor input = makeATenTensor<tc::CudaBackend>({1, 3, 226, 226}); // 1x3x224x224, padding = 1
    ...
    for (int i = 0; i < 15; i++) {
        ...
        std::cout << "resnet/conv_1x3x224x224_32x3x3x3_S2P1[Round" << i <<"]: " << time_use << " us" << std::endl;
    }
	time_use_sum /= 10;
	std::cout << "resnet/conv_1x3x224x224_32x3x3x3_S2P1[Time]: " << time_use_sum << " us" << std::endl;

    /***********************conv_1x32x112x112_32x32x3x3_S1P1*********************/
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 32, 114, 114}); // 1x3x112x112, padding = 1
    kernel = makeATenTensor<tc::CudaBackend>({32, 32, 3, 3});
    ...
    for (int i = 0; i < 15; i++) {
        ...
        // cudaEventElapsedTime(&time_use, start, end);
        if (i >= 5) time_use_sum += time_use;
        std::cout << "resnet/conv_1x32x112x112_32x32x3x3_S1P1[Round" << i <<"]: " << time_use << " us" << std::endl;
    }
	time_use_sum /= 10;
	std::cout << "resnet/conv_1x32x112x112_32x32x3x3_S1P1[Time]: " << time_use_sum << " us" << std::endl;

    /***********************conv_1x32x112x112_64x32x3x3_S1P1*********************/
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 32, 114, 114}); // 1x3x112x112, padding = 1
    kernel = makeATenTensor<tc::CudaBackend>({64, 32, 3, 3});
    pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S2_string, "convolution", {input, kernel}, bestOptions_S2[0]);
    for (int i = 0; i < 15; i++) {
        // cudaEventRecord(start, 0);
        ...
        // cudaEventElapsedTime(&time_use, start, end);
        if (i >= 5) time_use_sum += time_use;
        std::cout << "resnet/conv_1x32x112x112_64x32x3x3_S1P1[Round" << i <<"]: " << time_use << " us" << std::endl;
    }
	time_use_sum /= 10;
	std::cout << "resnet/conv_1x32x112x112_64x32x3x3_S1P1[Time]: " << time_use_sum << " us" << std::endl;
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 128, 56, 56}); // 1x128x56x56, padding = 1
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 128, 30, 30}); // 1x512x28x28, padding = 0
    kernel = makeATenTensor<tc::CudaBackend>({128, 128, 3, 3});
    pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S1_string, "convolution", {input, kernel}, bestOptions_S1[0]);
    for (int i = 0; i < 15; i++) {
        // cudaEventRecord(start, 0);
    ...
    /***********************conv_1x256x28x28_256x256x3x3_S1P1*********************/
    time_use_sum = 0;
    ...
    for (int i = 0; i < 15; i++) {
        ...
        // cudaEventElapsedTime(&time_use, start, end);
        if (i >= 5) time_use_sum += time_use;
        std::cout << "resnet/conv_1x256x28x28_256x256x3x3_S1P1[Round" << i <<"]: " << time_use << " us" << std::endl;
    }
	time_use_sum /= 10;
	std::cout << "resnet/conv_1x256x28x28_256x256x3x3_S1P1[Time]: " << time_use_sum << " us" << std::endl;
    ...
    /***********************conv_1x512x28x28_1024x512x1x1_S1P0*********************/
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 512, 28, 28}); // 1x512x28x28, padding = 0
    ...
    for (int i = 0; i < 15; i++) {
        ...
        // cudaEventElapsedTime(&time_use, start, end);
        if (i >= 5) time_use_sum += time_use;
        std::cout << "resnet/conv_1x512x28x28_1024x512x1x1_S1P0[Round" << i <<"]: " << time_use << " us" << std::endl;
    }
	time_use_sum /= 10;
	std::cout << "resnet/conv_1x512x28x28_1024x512x1x1_S1P0[Time]: " << time_use_sum << " us" << std::endl;
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 256, 16, 16}); // 1x256x16x16, padding = 1
    kernel = makeATenTensor<tc::CudaBackend>({256, 256, 3, 3});
    pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S1_string, "convolution", {input, kernel}, bestOptions_S1[0]);
    for (int i = 0; i < 15; i++) {
        // cudaEventRecord(start, 0);
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 1024, 14, 14}); // 1x512x7x7, padding = 0
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 1024, 14, 14}); // 1x2048x7x7, padding = 0
    kernel = makeATenTensor<tc::CudaBackend>({2048, 1024, 1, 1});
    pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S1_string, "convolution", {input, kernel}, bestOptions_S1[0]);
    for (int i = 0; i < 15; i++) {
        // cudaEventRecord(start, 0);
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 512, 9, 9}); // 1x2048x7x7, padding = 1
    kernel = makeATenTensor<tc::CudaBackend>({512, 512, 3, 3});
    pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S1_string, "convolution", {input, kernel}, bestOptions_S1[0]);
    for (int i = 0; i < 15; i++) {
        // cudaEventRecord(start, 0);
