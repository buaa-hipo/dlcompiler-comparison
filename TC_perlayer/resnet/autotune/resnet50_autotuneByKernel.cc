...
// 不再重用调优结果, 而是针对每个kernel进行调优
DEFINE_string(
    options_cache,
    "autotune_msg_resnet50_autotuneByKernel",
...
{
    // auto options1 = Backend::MappingOptionsType::makeNaiveMappingOptions();
    // std::vector<typename Backend::MappingOptionsType> options2 {options1};
    // return options2;
    // 尝试从保存的文件中获取调优信息
    auto options = \
...
TEST(resnet, conv)
{
    ...
    auto bestOptions = get_convolution_bestOption<tc::CudaBackend>(conv_S2_string, input, kernel);
    auto pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S2_string, "convolution", {input, kernel}, bestOptions[0]);
    for (int i = 0; i < 15; i++) {
    ...
    bestOptions = get_convolution_bestOption<tc::CudaBackend>(conv_S1_string, input, kernel);
    pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S1_string, "convolution", {input, kernel}, bestOptions[0]);
    for (int i = 0; i < 15; i++) {
        // cudaEventRecord(start, 0);
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 256, 56, 56}); // 1x64x56x56, padding = 0
    ...
    bestOptions = get_convolution_bestOption<tc::CudaBackend>(conv_S2_string, input, kernel);
    pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S2_string, "convolution", {input, kernel}, bestOptions[0]);
    for (int i = 0; i < 15; i++) {
        // cudaEventRecord(start, 0);
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 128, 30, 30}); // 1x128x30x30, padding = 1
    kernel = makeATenTensor<tc::CudaBackend>({128, 128, 3, 3});
    bestOptions = get_convolution_bestOption<tc::CudaBackend>(conv_S1_string, input, kernel);
    pExecutor = tc::aten::compile<tc::CudaBackend>(conv_S1_string, "convolution", {input, kernel}, bestOptions[0]);
    for (int i = 0; i < 15; i++) {
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 256, 30, 30}); // 1x256x28x28, padding = 0
    ...
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 1024, 14, 14}); // 1x1024x28x28, padding = 0
    ...
    /***********************conv_1x512x14x14_512x512x3x3_S1P1*********************/
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 512, 16, 16}); // 1x512x16x16, padding = 1
    ...
    for (int i = 0; i < 15; i++) {
        ...
        // cudaEventElapsedTime(&time_use, start, end);
        if (i >= 5) time_use_sum += time_use;
        std::cout << "resnet/conv_1x512x14x14_512x512x3x3_S1P1[Round" << i <<"]: " << time_use << " us" << std::endl;
    }
	time_use_sum /= 10;
	std::cout << "resnet/conv_1x512x14x14_512x512x3x3_S1P1[Time]: " << time_use_sum << " us" << std::endl;
    ...
    /***********************conv_1x1024x14x14_2048x1024x1x1_S1P0*********************/
    time_use_sum = 0;
    input = makeATenTensor<tc::CudaBackend>({1, 1024, 14, 14}); // 1x1024x14x14, padding = 0
    ...
    for (int i = 0; i < 15; i++) {
        ...
        // cudaEventElapsedTime(&time_use, start, end);
        if (i >= 5) time_use_sum += time_use;
        std::cout << "resnet/conv_1x1024x14x14_2048x1024x1x1_S1P0[Round" << i <<"]: " << time_use << " us" << std::endl;
    }
	time_use_sum /= 10;
	std::cout << "resnet/conv_1x1024x14x14_2048x1024x1x1_S1P0[Time]: " << time_use_sum << " us" << std::endl;
